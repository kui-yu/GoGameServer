...
func (this *ExtRobotClient) ResetBetArea(count int32) {
	...
	if count > 30 {
		//下注区域比率
		for i := 0; i < 60; i++ {
			this.AreaList = append(this.AreaList, INDEX_DRAGON)
			this.AreaList = append(this.AreaList, INDEX_TIGER)
		...
		for i := 0; i < 10; i++ {
			this.AreaList = append(this.AreaList, INDEX_DRAW)
			this.AreaList = append(this.AreaList, INDEX_DRAGONSPADE)
			this.AreaList = append(this.AreaList, INDEX_DRAGONPLUM)
			this.AreaList = append(this.AreaList, INDEX_DRAGONRED)
			this.AreaList = append(this.AreaList, INDEX_DRAGONBLOCK)
			this.AreaList = append(this.AreaList, INDEX_TIGERSPADE)
			this.AreaList = append(this.AreaList, INDEX_TIGERPLUM)
			this.AreaList = append(this.AreaList, INDEX_TIGERRED)
			this.AreaList = append(this.AreaList, INDEX_TIGERBLOCK)
		}
	}
	this.PlayList = []int{}
	//下注筹码比率
...
func (this *ExtRobotClient) HandlerGameEnd(d string) {
	...
	this.GameIn = false
	// //通知可以离开
	// controller.sendEvent(EVENT_CONT_ROBOTIDLE, this) // 千万不要写这种p话
}
