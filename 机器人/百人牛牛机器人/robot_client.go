...
func (this *RobotClient) RecvMessageThread() {
	go func() {
		for {
			...
			if id != MSG_HALL_HEART_REPLY {
				var fun func(int, interface{})
				for _, v := range this.TimeTicker.TList {
					if this.CheckOnLineId == v.Id {
						fun = v.H
					}
				}
				this.TimeTicker.DelTimer(this.CheckOnLineId)
				this.CheckOnLineId = this.TimeTicker.AddTimer(30, fun, nil)
			}
			if this.IsRobotStop() {
				return
			}
