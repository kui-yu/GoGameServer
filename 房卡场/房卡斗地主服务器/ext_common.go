...
const (
	...
	CT_BOMB_FOUR_SOFT      = 13 //软炸
	CT_BOMB_FOUR           = 14 //炸弹
	CT_BOME_FOUR_UP        = 15 //四张炸弹以上
	CT_TWOKING             = 16 //对王类型
)
...
func (this *MgrCard) Shuffle() {
	...
	// this.MSendId = 0
	// vd := []byte{}
	// vd = append(vd, Card_Hei_3, Card_Hong_3, Card_Mei_3, Card_Fang_3, Card_Mei_4, Card_Hong_4, Card_Hei_4)
	// vd = append(vd, Card_Fang_4, Card_Hei_5, Card_Mei_5, Card_Fang_5, Card_Hong_5, Card_Hei_6, Card_Mei_6)
	// vd = append(vd, Card_Fang_6, Card_Hong_6, Card_Hei_7)

	// vd = append(vd, Card_Hei_8, Card_Hong_8, Card_Mei_8, Card_Fang_8, Card_Mei_9, Card_Fang_9, Card_Hei_9)
	// vd = append(vd, Card_Hong_9, Card_Hei_10, Card_Hong_10, Card_Fang_10, Card_Mei_10, Card_Fang_J, Card_Mei_J)
	// vd = append(vd, Card_Hei_J, Card_Hong_J, Card_Hong_7)

	// vd = append(vd, Card_Hei_Q, Card_Hong_Q, Card_Mei_Q, Card_Fang_Q, Card_Mei_K, Card_Fang_K, Card_Hong_K)
	// vd = append(vd, Card_Hei_K, Card_Hei_1, Card_Hong_1, Card_Fang_1, Card_Mei_1, Card_Fang_2, Card_Mei_2)
	// vd = append(vd, Card_Hei_2, Card_Hong_2, Card_Mei_7)

	// vd = append(vd, Card_King_1, Card_King_2, Card_Fang_7)
	// this.MVSourceCard = []byte{}
	// this.MVSourceCard = append(this.MVSourceCard, vd...)
	//////////////////////
	this.MSendId = 0
...
//三带二，单顺
func Gen5OutCard(cards []byte, out *GOutCard, SanDaiEr bool) bool {
	c := Sort(cards)
	if (GetLogicValue(c[0]) == GetLogicValue(c[2])) &&
	...
	if (GetLogicValue(c[0]) == GetLogicValue(c[1])) &&
		(GetLogicValue(c[2]) == GetLogicValue(c[4])) && SanDaiEr {
		out.Max = c[2]
		out.Cards = append([]byte{}, c...)
		out.Type = CT_THREE_LINE_TAKE_TWO
		return true
...
//单顺，双顺，飞机带单,四带对
func Gen8OutCard(cards []byte, out *GOutCard, SiDaiEr bool) bool {
	c := Sort(cards)
	if GenDanShun(c, out) {
		out.Max = c[0]
		out.Cards = append([]byte{}, c...)
	...
	if (GetLogicValue(c[0]) == GetLogicValue(c[3])) &&
		...
		(GetLogicValue(c[4]) != GetLogicValue(c[6])) && SiDaiEr {
		out.Max = c[0]
		out.Cards = append([]byte{}, c...)
...
func DoGenCard(cards []byte, out *GOutCard, SanDaiEr bool, SiDaiEr bool) bool {
	re := false
	num := len(cards)
	if num == 0 {
		return false
	...
	case 5:
		re = Gen5OutCard(cards, out, SanDaiEr)
	case 6:
		re = Gen6OutCard(cards, out)
	case 7:
		re = Gen7OutCard(cards, out)
	case 8:
		re = Gen8OutCard(cards, out, SiDaiEr)
	default:
...
func DoGenCard2(cards []byte, num, style int32, out *GOutCard, SanDaiEr bool, SiDaiEr bool) bool {
	re := false
	switch num {
	case 1:
		re = Gen1OutCard(cards, out)
