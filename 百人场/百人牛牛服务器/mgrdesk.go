...
////////////////////////////////////
//风控算法
//var PD int64 = 100000 //每日库存要求
var CD int64 = 0 //当前库存
var StartControlTime = time.Now().Unix()

func CalPk1(pt, ct int64) int64 {
	return (ct - pt) / 86400 * G_DbGetGameServerData.GameConfig.GoalStock
}

func CalPk2(pt, ct int64) int64 {
	return (ct - pt) % 86400 / 60 * (G_DbGetGameServerData.GameConfig.GoalStock / 1440)
}

//目标库存
func CalPkAll(pt, ct int64) int64 {
	return CalPk1(pt, ct) + CalPk2(pt, ct)
}

func AddCD(coin int64) {
	CD += coin
	//logs.Debug("当前库存:%v", CD)
}

//如果PD - CalPkAll(pt, ct int64)>= 0就随机，反正就庄家赢
...
// func (this *DeskMgr) SetZouShi(zs *GameZouShi) {
// 	this.Zs = zs
// }
...
func (this *DeskMgr) SendNativeMsgNoPlayer(id int, uid int64, toid int32, d interface{}) {
	this.SendNativeMsg(id, uid, d)
}
...
func (this *DeskMgr) InitHandle() {
	...
	//
	this.Handle[MSG_GAME_GETZOUSHI] = this.GetZouShi
}
...
func (this *DeskMgr) GetZouShi(msg *DkInMsg) {
	this.Free[0].GetZouShi(0)
	// if this.Zs == nil {
	// 	return
	// }
	// //
	// req := GGetZouShi{}
	// json.Unmarshal([]byte(msg.Data), &req)
	// //
	// rsp := GGetZouShiReply{
	// 	Id:   MSG_GAME_GETZOUSHI_REPLY,
	// 	Data: *this.Zs,
	// }
	// rsp.Data.PlayerNum = int32(len(this.MapPlayers))
	...
	// //
	// this.SendNativeMsg(MSG_GAME_GETZOUSHI_REPLY, 0, &rsp)
}
...
func GetGameInfoFromDb() {
	if DB_SAVE_MODEL == 1 {
		...
		//目标库存测试
		G_DbGetGameServerData.GameConfig.GoalStock = 30000
	}
