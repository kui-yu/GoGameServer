...
func (this *DeskMgr) GameAuto(msg *DkInMsg) {
	...
	if GCONFIG.RoomType == 3 { //百人场
		...
		newfree := []*ExtDesk{}
		// logs.Debug("有桌子：", this.All[0].Mark, this.All[0].GameState, "空闲桌子：", this.Free[0].GameState)
...
func (this *DeskMgr) CreateFkDesk(msg *DkInMsg) {
	//没有空余的桌子
	if len(this.Free) == 0 {
		this.SendNativeMsg(MSG_GAME_FK_CREATEDESK_REPLY, msg.Uid, &GFkCreateDeskReply{
			Id:     MSG_GAME_FK_CREATEDESK_REPLY,
			Result: 1,
			Err:    "没有空余的桌子，请稍后重试",
		})
		return
