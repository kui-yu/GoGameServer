...
const (
	...
	MSG_HALL_GETZOUSHI              = 300018 //获取走势
	MSG_HALL_GETZOUSHI_REPLY        = 300019
	MSG_HALL_GETZOUSHI_SINGLE       = 300020 //获取单个走势
	MSG_HALL_GETZOUSHI_SINGLE_REPLY = 300021 //
)
...
const (
	...
	MSG_GAME_GETZOUSHI           = 400025 //获取走势图
	MSG_GAME_GETZOUSHI_REPLY     = 400026
)
...
type HMsgHallLoginReply struct {
	...
	CurrentRechargeAmount int64
	NextRechargeAmount    int64
	NextVipId             int64
	NextVipName           string
}
...
type HMsgHallGetZouShiReply struct {
	Id     int32
	Result int32
	Err    string
	ZouShi []GameZouShi
...
type HMsgHallGetZouShiSingle struct {
	Id        int32
	GameType  int32 //游戏类型
	RoomType  int32 //房间类型
	GradeType int32 //场次类型
...
type HMsgHallGetZouShiSingleReply struct {
	Id     int32
	Result int32
	Err    string
	ZouShi GameZouShi
...
type GAutoGameToHall struct {
	...
	RoomType  int32
	GradeType int32
	SerId     int32 //0:随机选择，其他主动选择哪台服务器
}
...
type GLeaveReply struct {
	...
	Token   string
	Robot   bool // 是否机器人
	NoToCli bool //false:发给客户端，true，不发客户端
}
...
type GGameEnd struct {
	...
	NoSaveCoin  int32          //修改玩家金币，0修改，1不修改
...
type GGetZouShi struct {
	Id     int32
	FromId int32
}

type GGetZouShiReply struct {
	Id   int32
	Data GameZouShi
}

type GameZouShi struct {
	SerId       int32          //服务器id
	GameInfo    GameTypeDetail //游戏类型
	Data        GameData
	UpdateT     int64 //更新时间
	GradeNumber int32 //场次编号
	GateId      int32
}
type GameData struct {
	ZouShi     string //走势
	PlayerNum  int32  //玩家人数
...
type DbPlayerData struct {
	...
	CurrentRechargeAmount int64   `json:"currentRechargeAmount"`
	NextRechargeAmount    int64   `json:"nextRechargeAmount"`
	NextVipId             int64   `json:"nextVipId"`
	NextVipName           string  `json:"nextVipName"`
}
...
type DbGameServerConfig struct {
	...
	ChipMax          int64                `json:"chipMax"`          //筹码最大值
	ChipMin          int64                `json:"chipMin"`          //筹码最小值
	LimitRedMax      int64                `json:"limitRedMax"`      //限红最大值
	CumulativeStock  int64                `json:"cumulativeStock"`  //累计库存
	TenChips         []int64              `json:"tenChips"`         //十个筹码值
	ProfitValue      float64              `json:"profitValue"`      //盈利值
	VIPLimitRed      int64                `json:"vipLimitRed"`      //VIP限红
	DifferentBetArea []int                `json:"differentBetArea"` //不同下注区域
}
