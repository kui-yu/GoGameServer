...
func (this *ExtDesk) HandleTimeOutBet(d interface{}) {
	...
	if this.NewBet {
		...
		for _, p := range this.Players {
			...
			// p.ColAreaCoins()
			// 玩家总下注
			// msg.PAreaCoins = p.GetTotBetList()
...
func (this *ExtDesk) HandleGameBet(p *ExtPlayer, d *DkInMsg) {
	...
	// 对、顺、金
	case INDEX_BANKER_DOUBLE:
	case INDEX_BANKER_SHUNZI:
	case INDEX_BANKER_JINHUA:

	// 顺金、炸弹
	case INDEX_BANKER_SHUNJIN:
	case INDEX_BANKER_BAOZI:
	default:
