...
func (this *ExtDesk) TimerStopBet(d interface{}) {
	...
	PD := G_DbGetGameServerData.GameConfig.GoalStock
	logs.Debug("*************开始控制时间:%v", StartControlTime)
	logs.Debug("每日库存需求:%v; 当前库存:%v; 目标库存:%v; 当前时间:%v;", PD, CD, CalPkAll(StartControlTime, currentTime), currentTime)
	Rcard, Rcolor := SortHandCard(this.RedCard)   //将红方的牌进行排序并分出花色，数值
	Bcard, Bcolor := SortHandCard(this.BlackCard) //将黑方的牌进行排序并分出花色，数值
	chart, typeChart := this.CardMgr.CompareCard(Rcard, Rcolor, Bcard, Bcolor)
	WinArea, TWinArea := this.Predict(chart, typeChart)
	logs.Debug("赢取区域:%v ; 总输赢: %v", WinArea, TWinArea)
	if CD-CalPkAll(StartControlTime, currentTime) < 0 {
		for _, v := range playersList {
			var betAreaMaxId int
			var WinMaxId int
			var pw int
			var pb int
			var redCard []int32
			var blackCard []int32
			p := v.(*ExtPlayer)
			playerBetArea := p.GetNTAreaCoinsList()
			//获取玩家赢的区域
			playWinList := p.GetWinList()
			logs.Debug("玩家昵称:%v ; 玩家下注区域:%v; 玩家赢取区域:%v;", p.Nick, playerBetArea, playWinList)
			for i := 1; i < len(playerBetArea); i++ {
				if playerBetArea[i] > playerBetArea[pb] {
					pb = i
				}
			}
			betAreaMaxId = pb
			for i := 1; i < len(playWinList); i++ {
				if playWinList[i] > playWinList[pw] {
					pw = i
				}
			}
			WinMaxId = pw
			if playWinList[betAreaMaxId] > 0 {
				if playWinList[WinMaxId] >= playWinList[betAreaMaxId] {
					logs.Debug("区域ID为:%v", WinMaxId)
					redCard, blackCard = this.BuildCard(WinMaxId)
					this.RedCard = redCard
					this.BlackCard = blackCard
				}
				logs.Debug("**************区域ID为:%v", betAreaMaxId)
				redCard, blackCard = this.BuildCard(betAreaMaxId)
				this.RedCard = redCard
				this.BlackCard = blackCard
			} else {
				if playWinList[WinMaxId] > 0 {
					logs.Debug("++++++++++++++区域ID为:%v", WinMaxId)
					redCard, blackCard = this.BuildCard(WinMaxId)
					this.RedCard = redCard
					this.BlackCard = blackCard
