...
func (this *ExtDesk) HandleReconnect(p *ExtPlayer, d *DkInMsg) {
	...
	for _, v := range this.Players {
		...
		if v.IsLook {
			...
			status.PlayPoints = append(status.PlayPoints, v.NiuPoint)
			var niuCards []int32
			for _, sv := range v.NiuCards {
				niuCards = append(niuCards, int32(sv))
			}
			status.PlayCards = append(status.PlayCards, niuCards)
		}
	}
