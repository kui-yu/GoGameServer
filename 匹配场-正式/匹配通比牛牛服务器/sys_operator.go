...
func (this *ExtDesk) ControlResult() (winChairs []int) {
	rand.Seed(time.Now().Unix())
	RobotList := this.GetRobot()
	if len(RobotList) == 0 || GetCostType() == 2 || CD-CalPkAll(StartControlTime, time.Now().Unix()) >= 0 {
		logs.Debug("条件不足，将不会执行风控")
		for index, _ := range this.Players {
			winChairs = append(winChairs, index)
		}
		winChairs = ListShuffleByInt(winChairs)
	...
	if CD-CalPkAll(StartControlTime, time.Now().Unix()) < 0 {
		logs.Debug("当前库存小于目标库存，执行风控")
		for index, v := range this.Players {
			if v.Robot {
				robotIndex = append(robotIndex, index)
			} else {
				playerIndex = append(playerIndex, index)
			}
		}

		robotIndex = ListShuffleByInt(robotIndex)
		winChairs = append(winChairs, robotIndex[0])
		if len(robotIndex) > 1 {
			robotIndex = robotIndex[1:]
			playerIndex = append(playerIndex, robotIndex...)
		} else {
			robotIndex = []int{}
		}
		indexss := rand.Perm(len(playerIndex))
		newPlayer := []int{}
		for _, v := range indexss {
			newPlayer = append(newPlayer, playerIndex[v])
		}
		winChairs = append(winChairs, newPlayer...)
	}
	return
}
